export type QualityCoreSqlOptions = {
  schemaName?: string;
  dropSchema?: boolean;
};

export function buildQualityCoreMigrationSql(options: QualityCoreSqlOptions = {}): string[] {
  const schemaName = normalizeIdentifier(options.schemaName ?? "quality_core", "schemaName");
  return [
    ...
    `CREATE UNIQUE INDEX IF NOT EXISTS ${getQualityCoreLookupIndexName()} ON ${schemaName}.primary_records (tenant_id, title, correlation_id);`,
    `CREATE INDEX IF NOT EXISTS ${getQualityCoreStatusIndexName()} ON ${schemaName}.exception_records (tenant_id, status, severity);`
  ];
}

export function buildQualityCoreRollbackSql(options: QualityCoreSqlOptions = {}): string[] {
  const schemaName = normalizeIdentifier(options.schemaName ?? "quality_core", "schemaName");
  const dropSchema = options.dropSchema ?? schemaName !== "quality_core";
  return [
...
export function getQualityCoreLookupIndexName(): string {
  return "quality_core_primary_lookup_idx";
}

export function getQualityCoreStatusIndexName(): string {
  return "quality_core_exception_status_idx";
}
