export type QualityCoreSqliteOptions = {
  tablePrefix?: string;
};

export function buildQualityCoreSqliteMigrationSql(options: QualityCoreSqliteOptions = {}): string[] {
  const tablePrefix = normalizePrefix(options.tablePrefix ?? "quality_core_");
  return [
    ...
    `CREATE UNIQUE INDEX IF NOT EXISTS ${getQualityCoreSqliteLookupIndexName(tablePrefix)} ON ${tablePrefix}primary_records (tenant_id, title, correlation_id);`,
    `CREATE INDEX IF NOT EXISTS ${getQualityCoreSqliteStatusIndexName(tablePrefix)} ON ${tablePrefix}exception_records (tenant_id, status, severity);`
  ];
}

export function buildQualityCoreSqliteRollbackSql(options: QualityCoreSqliteOptions = {}): string[] {
  const tablePrefix = normalizePrefix(options.tablePrefix ?? "quality_core_");
  return [
...
export function getQualityCoreSqliteLookupIndexName(tablePrefix = "quality_core_"): string {
  return `${normalizePrefix(tablePrefix)}primary_lookup_idx`;
}

export function getQualityCoreSqliteStatusIndexName(tablePrefix = "quality_core_"): string {
