...
import { businessFlowDefinitions, recordInspection, applyQualityHold, openCapa, placeRecordOnHold, releaseRecordHold, amendRecord, reverseRecord } from "../../src/flows/catalog";
...
describe("plugin manifest", () => {
  it("keeps a stable package id and business contract surface", () => {
    expect(manifest.id).toBe("quality-core");
    expect(manifest.kind).toBe("plugin");
    expect(manifest.publicCommands).toContain("quality.inspections.record");
    expect(manifest.publicEvents).toContain("quality.inspection-recorded.v1");
  });
});
...
describe("domain catalog", () => {
  it("keeps ERPNext parity references and operational surfaces visible", () => {
    ...
    expect(typeof recordInspection).toBe("function");
    expect(typeof applyQualityHold).toBe("function");
    expect(typeof openCapa).toBe("function");
